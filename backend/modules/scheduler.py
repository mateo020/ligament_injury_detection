...
class GaussianNoiseScheduler(BasicNoiseScheduler):
    ...
    def estimate_x_t(self, x_0, t, x_T=None):
        """ 
        Foward noising step. 
        takes clean sample x0 and returns noisy version at timestep t denoted xt
        alpha bar t is the cumulative product of 𝛼 α–coefficients up to timestep 𝑡t.
        ...
        def clipper(b):
            ...
            if tb < 0:
                ...
                x_0[b] *= self.sqrt_alphas_cumprod[tb]
                x_0[b] += self.sqrt_one_minus_alphas_cumprod[tb] * x_T[b]
                return x_0[b]
    ...
    def estimate_x_t_prior_from_x_T(self, x_t, t, x_T, use_log=True, clip_x0=True,  var_scale=0, cold_diffusion=False): 
        """
        leverages your existing estimate_x_0 + estimate_x_t_prior_from_x_0 logic to invert one diffusion step when your network predicts the pure-noise component X_t
        Given noise tensor xt at timestep t and the corresposning pure noise tensor xt, recoers prior estimate for the previous timestep xt-1
        """
        x_0 = self.estimate_x_0(x_t, x_T, t, clip_x0) 
        return self.estimate_x_t_prior_from_x_0(x_t, t, x_0, use_log, clip_x0, var_scale, cold_diffusion) #once you have the estimate of x0 (clean signal) we compute posterior prior x t-1


    def estimate_x_t_prior_from_x_0(self, x_t, t, x_0, use_log=True, clip_x0=True, var_scale=0, cold_diffusion=False):
        """
         method implements the core reverse‐diffusion step—recovering the previous noisy state x_t-1 from the current noisy observation xt
         once you have a prediction of the clean data

         tandard DDPM reverse‐process (stochastic denoising)

         Cold diffusion correction (deterministic inversion of arbitrary degradations)
