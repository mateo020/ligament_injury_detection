...
class DiffusionPipeline(BasicModel):
    ...
    def _step(self, batch, batch_idx, state, step):
        """
        Runs one trainig step 
        - Embed x0 into latent space to normalize
        - Applies noise to generate x_t and x_T at random timestep t
        - runs the model (noise estimator to predict the mean of the noise )
        - comptes a loss btween predicted and actual 
        - if variance is learned adds KL divergence or Gausssion MLLL loss
    ...
    def forward(self, x_t, t, condition=None, self_cond=None, guidance_scale=1.0, cold_diffusion=False, un_cond=None):
        """
        Input noisy image xt and time step t

        single step inversion of diffusion process:
         - predicts the noise with optional guidance 
         - componse next denoised sample
        """
